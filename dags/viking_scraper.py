...
class Scraper:
    def __init__(
        ...
        self._browser = pw.chromium.launch(headless=True, slow_mo=50)
        self._config = config
        self.logger = logger
    ...
    def _extract_subscription_data(self):
        ...
        try:
            ...
            for subscription_element in subscription_elements:
                mobile_data = subscription_element.select_one(self._config['selector']['mobile_subscription']['data']).get_text().lower().replace('gb', '').strip()
                calls_texts = subscription_element.select_one(self._config['selector']['mobile_subscription']['calls_texts']).get_text().lower()
                price_per_month = subscription_element.select_one(self._config['selector']['mobile_subscription']['price']).get_text().strip().replace(',-', '')
    ...
    def _extract_internet_table_data(self):
        ...
        try:
            price = soup.select_one(self._config['selector']['internet_subscription']['price']).get_text().strip()
            cleaned_price = price.replace(',-', '')

            monthly_data = soup.select_one(self._config['selector']['internet_subscription']['data']).get_text().lower()

            download_speed = soup.select_one(self._config['selector']['internet_subscription']['download_speed']).get_text().encode('ascii', 'ignore').decode('ascii').lower().strip()
            upload_speed = soup.select_one(self._config['selector']['internet_subscription']['upload_speed']).get_text().encode('ascii', 'ignore').decode('ascii').lower().strip()
...
def mobileviking_scraper(config):
    with sync_playwright() as pw:
        scraper_object = Scraper(pw, config)
